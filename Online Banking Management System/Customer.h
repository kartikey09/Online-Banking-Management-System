...
int isCustomerActive(int accNo);
int addActiveCustomer(int accNo);
void removeActiveCustomer(int accNo);
int isEmployeeActive(int accNo);
int addActiveEmployee(int accNo);
void removeActiveEmployee(int accNo);
...
int loginCustomer(int connectionFD, int accountNumber, char *password) {
    ...
    int file = open(CUSPATH, O_RDONLY); // Use O_RDONLY
    if (file == -1) {
        perror("loginCustomer: Could not open CUSPATH");
        return 0;
    }

    // --- MUTEX CHECK: Use the new helper function ---
    if (isCustomerActive(accountNumber)) {
        printf("Customer with account number %d is already logged in!\n", accountNumber);
        close(file);
        return 0;
    }

    // --- BUG FIX: Trim newline from password ---
    password[strcspn(password, "\r\n")] = '\0';

    while(read(file, &customer, sizeof(customer)) == sizeof(struct Customer))
    {
        if (customer.accountNumber == accountNumber && strcmp(customer.password, crypt(password, HASHKEY)) == 0 && customer.activeStatus == 1) {
            ...
            if(addActiveCustomer(accountNumber)) {
    ...
    printf("Customer %d not found or password incorrect\n", accountNumber);
    close(file);
    return 0;
}
...
void addFeedback(int connectionFD){
    ...
    {
        strcpy(fb.feedback, "Unsatisfactory");
    }
...
// ======================= Logout =======================
void logout(int connectionFD, int id) {
    // This function is called by both Customers and Employees,
    // so we try to remove the ID from both tables. Only one will match.
    removeActiveCustomer(id);
    removeActiveEmployee(id);
    
    printf("ID %d logged out and removed from active list.\n", id);
...
// ======================= exitClient =======================
void exitClient(int connectionFileDescriptor, int id) {
    // This function is also called by both, so remove from both tables.
    removeActiveCustomer(id);
    removeActiveEmployee(id);

    printf("ID %d exited and removed from active list.\n", id);

    // Send final message to client
    bzero(writeBuffer, sizeof(writeBuffer));
    strcpy(writeBuffer, "Client logging out...\n");
    write(connectionFileDescriptor, writeBuffer, sizeof(writeBuffer));
    // Note: You may not get a read() back here as the client might just close
...
int isCustomerActive(int accNo) {
    int active = 0;
    pthread_mutex_lock(&customer_login_mutex); // Lock
    for (int i = 0; i < MAX_ACTIVE_USERS; i++) {
        if (active_customers[i] == accNo) {
            active = 1;
            break; 
        }
    }
    pthread_mutex_unlock(&customer_login_mutex); // Unlock
    return active;
}
...
int isEmployeeActive(int empID) {
    int active = 0;
    pthread_mutex_lock(&employee_login_mutex); // Lock
    for (int i = 0; i < MAX_ACTIVE_USERS; i++) {
        if (active_employees[i] == empID) {
            active = 1;
            break; 
        }
    }
    pthread_mutex_unlock(&employee_login_mutex); // Unlock
    return active;
}

int addActiveEmployee(int empID) {
    int success = 0;
    pthread_mutex_lock(&employee_login_mutex); // Lock
    for (int i = 0; i < MAX_ACTIVE_USERS; i++) {
        if (active_employees[i] == 0) { 
            active_employees[i] = empID;
            success = 1;
            break;
        }
    }
...
void removeActiveEmployee(int empID) {
        pthread_mutex_lock(&employee_login_mutex); // Lock
    for (int i = 0; i < MAX_ACTIVE_USERS; i++) {
        if (active_employees[i] == empID) {
